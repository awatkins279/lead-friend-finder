...
// ---------------------------------------------------------------------------
// RingCentral WebRTC — mint a short-lived SIP-provision payload so the browser
// can register a WebPhone and place the call through the rep's mic/speakers.
// ---------------------------------------------------------------------------
const rcSipInput = z.object({
  phoneAccountId: z.string().uuid(),
});

export const getRingCentralSipProvision = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) => rcSipInput.parse(i))
  .handler(async ({ data, context }): Promise<{
    ...
    const provRes = await fetch(`${serverUrl}/restapi/v1.0/client-info/sip-provision`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Content-Type": "application/json",
    ...
    return {
      sipInfo: prov.sipInfo,
      sipFlags: prov.sipFlags,
      sipErrorCodes: prov.sipErrorCodes,
      fromNumber: acc.from_number,
      appKey: clientId,
    };
  });

// ---------------------------------------------------------------------------
// Create the `calls` row for a RingCentral WebRTC call (browser places the
// actual SIP INVITE — this just tracks it in our history).
// ---------------------------------------------------------------------------
const rcStartInput = z.object({
  listId: z.string().uuid(),
  leadId: z.string().min(1),
  phoneAccountId: z.string().uuid(),
  toNumber: z.string().min(3).max(32),
...
export const startRingCentralWebCall = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) => rcStartInput.parse(i))
  .handler(async ({ data, context }): Promise<{ callId: string }> => {
    const { supabase, userId } = context;
