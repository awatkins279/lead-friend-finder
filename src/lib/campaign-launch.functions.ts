...
async function instantlyRequest(apiKey: string, path: string, init: RequestInit) {
  ...
  if (!response.ok) {
    const message =
      payload?.message ?? payload?.error ?? `Instantly request failed (${response.status})`;
...
export const launchCampaign = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: list, error: listError } = await db
      .from("lists")
      .select(
        "id, name, user_id, campaign_status, instantly_campaign_id, unsubscribe_footer_enabled, unsubscribe_footer_text, sending_days, sending_start_time, sending_end_time, sending_timezone, follow_up_delay_days, email_gap_minutes",
      )
      .eq("id", data.listId)
    ...
    const [{ data: connection }, { data: mailboxRows }, { data: leadRows }] = await Promise.all([
      ...
      db
        ...
        .select("email_accounts(email_address, provider, status)")
        .eq("list_id", list.id),
      db
        .from("list_leads")
        .select(
    ...
    if (!connection?.api_key || connection.status !== "active")
      throw new Error("Connect Instantly under Sending accounts first");
    ...
    const sequences = [
      {
        steps: Array.from({ length: stepCount }, (_, index) => ({
          ...
          delay_unit: "days",
          variants: [
            { subject: `{{nexus_subject_${index + 1}}}`, body: `{{nexus_body_${index + 1}}}` },
          ],
        })),
      },
    ];
    const footer =
      list.unsubscribe_footer_enabled && list.unsubscribe_footer_text
          ...
          schedules: [
            {
              name: "Weekdays",
              timing: {
                from: String(list.sending_start_time).slice(0, 5),
                to: String(list.sending_end_time).slice(0, 5),
              },
              days: Object.fromEntries(
                Array.from({ length: 7 }, (_, day) => [
                  String(day),
                  (list.sending_days as number[]).includes(day),
                ]),
              ),
    ...
    if (typeof instantlyCampaignId !== "string")
    ...
    try {
      ...
      await instantlyRequest(connection.api_key, `/campaigns/${instantlyCampaignId}`, {
        method: "DELETE",
      }).catch(() => null);
      throw error;
    }
...
async function getConnection(db: any, userId: string) {
  const { data } = await db
    ...
    .eq("user_id", userId)
    .maybeSingle();
  if (!data?.api_key || data.status !== "active")
    throw new Error("Connect Instantly under Sending accounts first");
