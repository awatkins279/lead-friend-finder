...
export const pauseOperatorBlueprint = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await db.from("operator_events").update({ status: "paused" }).eq("blueprint_id", blueprint.id).eq("user_id", context.userId).eq("event_type", "operator_pipeline").in("status", ["running", "processing"]);
