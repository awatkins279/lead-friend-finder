...
function OrderPage() {
  ...
  useEffect(() => {
    loadOrders();
    // If we returned from Stripe, refresh shortly after (webhook needs a moment).
    if (new URLSearchParams(window.location.search).get("session_id")) {
      toast.success("Payment received — your order is being set up.");
  ...
  // ---- Payment view (embedded Stripe checkout) ----
  if (clientSecret) {
    return (
    ...
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Done-for-you domains + mailboxes, warmed and ready for cold outreach. We set everything up
