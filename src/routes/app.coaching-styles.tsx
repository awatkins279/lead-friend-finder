...
import { ColdCallPractice } from "@/components/ColdCallPractice";
import {
  listCoachingStyles,
  upsertCoachingStyle,
  deleteCoachingStyle,
...
function CoachingStylesPage() {
  ...
  const [tab, setTab] = useState<"styles" | "practice">("styles");
    ...
    <div className="space-y-6">
      <header className="glass-panel-strong flex items-center justify-between rounded-2xl p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold tracking-tight">Coaching</h1>
            <p className="text-xs text-muted-foreground">
              {isAdmin
                ? "Train the AI on sales methodologies or practice cold calls."
                : "Practice cold calls and view coaching styles."}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            <GraduationCap className="mr-1 h-4 w-4" /> Styles
          </Button>
          <Button
            variant={tab === "practice" ? "default" : "ghost"}
            size="sm"
            onClick={() => setTab("practice")}
            className={tab === "practice" ? "bg-gradient-to-r from-purple-600 to-blue-600" : ""}
          >
            <Phone className="mr-1 h-4 w-4" /> Practice
          </Button>
        </div>
      </header>
            ...
            <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
              ...
              <div className="space-y-4 py-2">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    This is the instructions the live AI follows during every call. Train it on the
                    style you want — tone, question patterns, framing rules, phrases to avoid, etc.
                  </p>
                  <Textarea
                    rows={10}
                    ...
                    <div key={i} className="space-y-1 rounded-lg border border-border/50 p-2">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          setEdit((s) => ({
                            ...s,
                            example_objection_handlers: s.example_objection_handlers.filter(
                              (_, j) => j !== i,
                            ),
                          }))
                        }
