...
BEGIN
  ...
  WITH filtered AS (
    SELECT l.id, l.first_name, l.last_name, l.email, l.phone, l.title,
           l.org_name, l.org_industry, l.org_employee_count,
           l.city, l.state, l.country, l.linkedin_url, l.imported_by,
           l.created_at, l.updated_at, l.source, l.user_id,
           l.org_description, l.org_technologies_used, l.org_founded_year,
           l.org_seo_description, l.org_website, l.org_logo_url, l.headline
    FROM public.leads l
    WHERE (l.imported_by IS NULL OR l.imported_by = p_user_id)
      ...
      AND (cardinality(v_loc_tokens) = 0 OR (
        (l.location_tokens IS NOT NULL AND l.location_tokens && v_loc_tokens)
        OR (l.location_tokens IS NULL AND EXISTS (
          SELECT 1 FROM unnest(v_loc_tokens) tok
          WHERE lower(coalesce(l.city,''))   = tok
             OR lower(coalesce(l.state,''))  = tok
             OR lower(coalesce(l.country,''))= tok
        ))
      ))
