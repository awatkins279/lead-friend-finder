...
BEGIN
  ...
  IF cardinality(v_raw_sizes) > 0 THEN
    SELECT coalesce(sum(bucket_count), 0)::integer INTO v_total_count
    FROM (
      SELECT l.org_employee_count, count(*) AS bucket_count
      FROM public.leads l
      WHERE l.imported_by IS NULL
        AND (v_name = '' OR ((array_length(regexp_split_to_array(v_name, '\s+'), 1) >= 2 AND (l.first_name ILIKE '%' || split_part(v_name, ' ', 1) || '%' OR l.last_name ILIKE '%' || split_part(v_name, ' ', 1) || '%') AND (l.first_name ILIKE '%' || substring(v_name from position(' ' in v_name) + 1) || '%' OR l.last_name ILIKE '%' || substring(v_name from position(' ' in v_name) + 1) || '%')) OR (array_length(regexp_split_to_array(v_name, '\s+'), 1) < 2 AND (l.first_name ILIKE '%' || v_name || '%' OR l.last_name ILIKE '%' || v_name || '%'))))
  ...
  v_total_count := coalesce(v_total_count, cardinality(v_ids));
